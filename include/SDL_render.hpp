...
#include <SDL_error.h>
	...
	public:
		...
		*/
		...
		{
			C::SDL_SetRenderTarget(*this, NULL);
		}
	...
	public:
		...
		bool createBlank(int width, int height, C::SDL_TextureAccess acc = C::SDL_TEXTUREACCESS_TARGET);
	...
	Texture::setAsRenderTarget() {
		C::SDL_SetRenderTarget(this->rend, *this);
	}

    __alwaysinline
	bool
	Texture::createBlank(int width, int height, C::SDL_TextureAccess acc) {
		//Create uninitialized texture

		C::SDL_Texture *tex = C::SDL_CreateTexture( this->rend, C::SDL_PIXELFORMAT_RGBA8888, acc, width, height );
		...
		if( this->ptr.get() == NULL )
		{
			printf( "Unable to create blank texture! SDL Error: %s\n", C::SDL_GetError() ); //TODO das muss noch raus nach ersten tests
		}

		return this->ptr.get() != NULL;
	}

	__alwaysinline
	Texture::Texture(Renderer &rend) : rend(rend){
